...
const SW_VERSION       = 'v7'; // bumped: pinch-zoom image viewer finalized
                                 // (touch-action rules in app.css + lightbox
                                 // JS, cache-busted app.css, debug code fully
                                 // removed) — new cache name forces a clean
                                 // refresh across all devices
...
// Icon URLs carry a ?v=3 cache-busting query string (bumped alongside the
// S/map-pin rebrand) so they must be precached under that exact URL —
// cache.match() keys on the full request including the query string, so
// listing the old bare filenames here would silently miss these and fall
// through to the network every time.
const SHELL_ASSETS = [
  './',
  './index.html',
  './manifest.json',
  ...
  './icon-72x72.png?v=3',
  './icon-96x96.png?v=3',
  './icon-128x128.png?v=3',
  './icon-144x144.png?v=3',
  './icon-152x152.png?v=3',
  './icon-192x192.png?v=3',
  './icon-384x384.png?v=3',
  './icon-512x512.png?v=3',
  './icon-maskable-192x192.png?v=3',
  './icon-maskable-512x512.png?v=3',
...
self.addEventListener('install', (event) => {
  ...
  // NOTE: no self.skipWaiting() here on purpose. A new worker must sit in
  // "waiting" so index.html's reg.waiting / updatefound logic can show the
  // update banner and let the user decide when to switch over. Only the
  // 'message' handler below (triggered by the "Update Now" button) should
  // ever call skipWaiting() for an update. First-ever installs (no prior
  // controller) still activate immediately regardless.
});
...
async function networkFirstShell(req) {
  ...
  try {
    // cache: 'reload' bypasses the browser's own HTTP cache layer, not
    // just the Service Worker cache — without this, a "network-first"
    // fetch() can still be silently satisfied by a stale HTTP-cached
    // copy of index.html if the host sends any caching headers on it.
